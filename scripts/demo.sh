#!/bin/bash
# RaftPay Demo Script
# Run this after `docker compose -f docker/docker-compose.yml up -d`
...
curl -s -X POST "$API_BASE:$LEADER_PORT/api/accounts" \
    ...
    -d '{"accountId":"alice","ownerName":"Alice Johnson","initialBalance":1000}' | python -m json.tool 2>/dev/null || echo "(created)"
echo ""

echo "  Creating Bob's account with \$500..."
curl -s -X POST "$API_BASE:$LEADER_PORT/api/accounts" \
    -H "Content-Type: application/json" \
    -d '{"accountId":"bob","ownerName":"Bob Smith","initialBalance":500}' | python -m json.tool 2>/dev/null || echo "(created)"
echo ""

echo "  Creating Charlie's account with \$250..."
curl -s -X POST "$API_BASE:$LEADER_PORT/api/accounts" \
    -H "Content-Type: application/json" \
    -d '{"accountId":"charlie","ownerName":"Charlie Brown","initialBalance":250}' | python -m json.tool 2>/dev/null || echo "(created)"
echo ""
...
curl -s -X POST "$API_BASE:$LEADER_PORT/api/transfers" \
    -H "Content-Type: application/json" \
    -d '{"fromAccountId":"alice","toAccountId":"bob","amount":100}' | python -m json.tool 2>/dev/null || echo "(done)"
echo ""

echo "  Bob -> Charlie: \$50"
curl -s -X POST "$API_BASE:$LEADER_PORT/api/transfers" \
    -H "Content-Type: application/json" \
    -d '{"fromAccountId":"bob","toAccountId":"charlie","amount":50}' | python -m json.tool 2>/dev/null || echo "(done)"
echo ""
...
echo "  1. Kill the leader:  bash scripts/kill-node.sh <N>"
echo "  2. New leader elected automatically"
echo "  3. Continue making transfers"
echo "  4. Restart node:     bash scripts/restart-node.sh <N>"
